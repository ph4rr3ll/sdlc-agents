...
5. **Группировка findings по severity:**

   - **CRITICAL**: блокирует merge. Безопасность, корректность, потеря данных, нарушение контракта.
   - **MAJOR**: должно быть исправлено до merge. Производительность, неполное покрытие AC, плохие тесты.
   - **MINOR**: желательно исправить. Читаемость, мелкие нарушения стиля.
   - **NIT**: предпочтения вкуса. Можно проигнорировать.

6. **Учёт OUT-OF-SCOPE OBSERVATIONS.** Если в `IMPL_NOTES` есть осознанные out-of-scope наблюдения — они НЕ считаются за findings (известны и приняты).
